...
contract KyberPrice {
	...
	function getOutputAmount(IERC20Token _from, IERC20Token _to, uint256 _amount, bytes32[] memory reserveIds) internal view returns (uint256) {
	...
	function getInputAmount(IERC20Token _from, IERC20Token _to, uint256 _amount, bytes32[] memory reserveIds) internal view returns (uint256) {
		uint256 initialAmount = getMultiplier(_from);
		uint256 initialReturn = getOutputAmount(_from, _to, initialAmount, reserveIds);
		if (initialReturn == 0) {
			return 0;
		}
		uint256 initialCost = _amount * initialAmount / initialReturn;
		uint256 finalReturn = getOutputAmount(_from, _to, initialCost, reserveIds);
		if (finalReturn == 0) {
			return 0;
		}
...
contract KyberRatesOracle is KyberPrice {
	function getOutputAmounts(IERC20Token _from, IERC20Token _to, uint256[100] memory _amounts, bytes32[] memory reserveIds) public view returns (uint256[100] memory amounts) {
        for (uint256 i = 0; i < _amounts.length; i++) {
			amounts[i] = getOutputAmount(_from, _to, _amounts[i], reserveIds);
		}
	}

	function getInputAmounts(IERC20Token _from, IERC20Token _to, uint256[100] memory _amounts, bytes32[] memory reserveIds) public view returns (uint256[100] memory amounts) {
		for (uint256 i = 0; i < _amounts.length; i++) {
			amounts[i] = getInputAmount(_from, _to, _amounts[i], reserveIds);
		}
	}
}